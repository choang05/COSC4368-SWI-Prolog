...
path(Start, End, Length, Path) :-
    travel(Start, End, [Start], Paths, Length),             % Recursivly travel
    reverse(Paths, Path).                                   % For each path in the Paths, print the path in reverse
...
travel(Start, End, Visited, Path, Length) :-
       ...
       travel(End2, End,[End2 | Visited], Path, Length3),    % travel with new connected.
